...
class ChannelListener:
    ...
    async def _on_message(self, event: events.NewMessage.Event) -> None:
        ...
        try:
            ...
            if self._last_processed.get(message.id) != text:
                log.info("MSG (msg_id=%d): message edited while parsing — discarding stale result", message.id)
                return
    ...
    async def _on_edit(self, event: events.MessageEdited.Event) -> None:
        ...
        try:
            ...
            # Stale check: if message was edited again while we were parsing, discard
            if self._last_processed.get(message.id) != text:
                log.info("EDIT (msg_id=%d): message changed while parsing — discarding stale result", message.id)
                return
            ...
            if self.notify:
                await self.notify(f"[{trade_manager.channel_name}] Error processing edited signal: {e}")
